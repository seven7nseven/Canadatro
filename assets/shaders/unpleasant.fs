...
extern PRECISION vec2 unpleasant;
...
vec4 effect(vec4 colour, Image texture, vec2 texcord, vec2 screencord) {
	...
	vec4 firstColor = vec4( 158./255.,  83./255.,   0.0,        1.0);
	vec4 middleColor = vec4(253./255.,  49./255.,   236./255.,  1.0);
	vec4 endColor = vec4(   36./255.,   214./255.,  40./255.,   1.0);
	
	vec2 xy = (((texcord)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba;

	number h = 0.5; // adjust position of middleColor
	vec4 col = mix(mix(firstColor, middleColor, xy.y/h), mix(middleColor, endColor, (xy.y - h)/(1.0 - h)), step(h, xy.y));
	
	vec4 canyoustopwhiningpleaseichangeditfromoutput = 2.0 * (mix(col, 2.0 * (col * vec4(0.55)), sin(unpleasant.x)) * src);
	return dissolve_mask(canyoustopwhiningpleaseichangeditfromoutput * colour, texcord, xy);
}
