...
SMODS.Joker({
	...
	loc_txt = {
		name = 'java duke',
		text = {'3 billion devices run java'}
	},
	atlas = 'duke',
	rarity = 1,
	cost = 3,
	...
	calculate = function(self, card, context)
		if context.joker_main then
			return {
				message = "3 billion devices run java"
			}
		end
	end,
})
...
--[[
G.videos = {}
function G.showFSVideo(path)
	local vid = {}
	local file = NFS.read(path)
	love.filesystem.write("temp.ogv", file)
	vid.video = love.graphics.newVideo('temp.ogv', { audio = true })
	vid.length = vid.video:getSource():getDuration()
	vid.width = vid.video:getWidth()
	vid.height = vid.video:getHeight()
...
function Game:update(dt)
	...
	if G.ARGS.save_run then G.ARGS.save_run['amen'] = G.amen end
end


local evilstart = Game.start_run
function Game:start_run(args)
	args = args or {}
	evilstart(self, args)
	if args.savetext then G.amen = args.savetext.amen else G.amen = false end
end
...
function love.draw()
	...
	--[[for _, v in pairs(G.videos) do
		love.graphics.draw(v.vid, 0, 0, 0, love.graphics.getWidth() / v.width, love.graphics.getHeight() / v.height)
	end]]
end
